...
class PremiumPDFGenerator:
    ...
    def _create_chart(self):
        ...
        for i in range(5):
            ax.plot([i, i], [0, 4], color='black', lw=1.5)
            ax.plot([0, 4], [i, i], color='black', lw=1.5)
        ...
        for h, (x, y) in house_coords.items():
            ...
            for i, (abbr, color) in enumerate(p_list):
                ax.text(x + x_offset, y + 0.4, abbr, color=color, ha='left', va='center', fontsize=8, fontweight='bold')
                x_offset += 0.25 # Horizontal spacing for multiple planets
                if x_offset > 0.8: # Wrap to next line if too many
                    x_offset = 0.2
                    y -= 0.2
                    
            ax.text(x + 0.1, y + 0.82, str(h), color='black', fontsize=7, alpha=0.6)
            if h == self.data.get('block'):
                ax.add_patch(plt.Rectangle((x, y), 1, 1, color='yellow', alpha=0.15))

        now = datetime.now().strftime("%d.%m.%Y - %H:%M")
        ax.text(2, 2, f"{now}\nRuling: {self.data.get('ruling_planet', '')}", color='black', ha='center', va='center', fontsize=8, fontstyle='italic')
    ...
    def generate(self):
        ...
        img = Image(self._create_chart(), width=3.5*inch, height=3.5*inch)
        story.append(img)
        ...
        story.append(PageBreak())
        for title, key in [("I. DIAGNOSTIC ANALYSIS (5 Unique Points)", "diagnostic_analysis"), 
                          ("II. RECOVERY TIMELINE (5 Unique Points)", "recovery_timeline"), 
                          ("III. REMEDIES & GUIDANCE (5 Unique Points)", "remedies")]:
            story.append(Paragraph(title, header_style))
            for i, pt in enumerate(self.data.get(key, [])[:5], 1):
                story.append(Paragraph(f"• {pt}", point_style))

        # 4.5 DETAILED SYNTHESIS ANALYSIS (Horary Logic)
        story.append(PageBreak())
        story.append(Paragraph("V. DETAILED SYNTHESIS ANALYSIS (Horary Logic)", header_style))
        s_points = self.data.get('synthesis_points', [])
        for pt in s_points:
            story.append(Paragraph(f"• {pt}", point_style))
        
        story.append(Spacer(1, 0.2*inch))
        story.append(Paragraph("Synthesis Conclusion:", ParagraphStyle('SubSub', fontSize=10, fontName='Helvetica-Bold', spaceBefore=6)))
        story.append(Paragraph(self.data.get('synthesis_conclusion', ""), ParagraphStyle('SynthC', fontSize=11, fontName='Helvetica-Oblique', textColor=self.brand_color_main, leftIndent=10)))
        ...
        story.append(Paragraph(self.data.get('final_conclusion', ""), ParagraphStyle('Final', fontSize=11, fontName='Helvetica-Bold')))
        ...
        story.append(Paragraph("Prepared by : RAJAGOPAL KANNAN   WhatsApp consultation : 9841033514", ParagraphStyle('F', fontSize=9, alignment=TA_CENTER, textColor=colors.grey, fontName='Helvetica-Bold')))
