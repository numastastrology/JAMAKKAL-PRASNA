...
class NatalEngine:
    ...
    def _calculate_ascendant(self, obs):
        """Calculates the Sidereal Ascendant by finding the rising ecliptic longitude."""

        def get_alt_az(ecl_lon_deg):
            body = ephem.Ecliptic(math.radians(ecl_lon_deg), 0)
            eq = ephem.Equatorial(body, epoch=obs.date)
            fb = ephem.FixedBody()
            fb._ra = eq.ra
            fb._dec = eq.dec
            fb._epoch = obs.date
            fb.compute(obs)
            return math.degrees(float(fb.alt)), math.degrees(float(fb.az))

        # Coarse scan: 1-degree steps, collect all near-horizon eastern candidates
        best_lon = 0
        best_score = 999
        for deg in range(360):
            alt, az = get_alt_az(deg)
            if abs(alt) < 3.0 and 50 < az < 130:
                score = abs(alt) + abs(az - 90) * 0.05
                if score < best_score:
                    best_score = score
                    best_lon = deg

        # Fine refinement: 0.01-degree steps around the best candidate
        fine_best = best_lon
        fine_score = 999
        for i in range(-200, 201):
            d = best_lon + i * 0.01
            alt, az = get_alt_az(d % 360)
            if 50 < az < 130:
                score = abs(alt)
                if score < fine_score:
                    fine_score = score
        ...
        return self._get_detailed_info(fine_best, self.dt)
